...
class ConvEdge(nn.Module):
    ...
    def forward(self, edge_fea, angle_fea, nbr_idx):
        ...
        N, M = nbr_idx.shape
        # print(f"nbr_idx shape N = {N}, M = {M}")
        # print(f"nbr_idx[0] = {nbr_idx[0]}")
        # print the neighbor lists
        neighbor_list_file = open("neighbor_list.txt", "w")
        for i in range(N):
            neighbor_list_file.write(f"atom index {i} neighbor list:\n")
            for j in range(M):
                neighbor_list_file.write(str(nbr_idx[i][j].item()))
                neighbor_list_file.write(' ')
            neighbor_list_file.write("\n")
        neighbor_list_file.close()
        ...
        # print(f"edge_fea_cat.size() = {edge_fea_cat.size()}")

        attention_1 = self.attention_1(edge_fea_cat)
        # print(f"attention_1.size() = {attention_1.size()}")
        # print(f"attention_1[0] = {attention_1[0]}")
        # print the attention_1 information to a new file
        attention_1_file = open("attention_1.txt", "w")
        dim1, dim2, dim3, dim4 = attention_1.size()
        for i in range(dim1):
            attention_1_file.write(f"atom index {i} attention_1 features:\n")
            for j in range(dim2):
                attention_1_file.write(f"neighbor {j}:\n")
                for k in range(dim3):
                    attention_1_file.write(str(attention_1[i][j][k].item()))
                    attention_1_file.write(' ')
                attention_1_file.write("\n")
        attention_1_file.close()
        alpha_1 = self.softmax_1(self.leakyrelu_1(attention_1))
        # print(f"alpha_1.size() = {alpha_1.size()}")
        # print(f"alpha_1[0] = {alpha_1[0]}")

        # print the alpha_1 information to a new file
        alpha_1_file = open("alpha_1.txt", "w")
        dim1_, dim2_, dim3_, dim4_ = alpha_1.size()
        for i in range(dim1_):
            alpha_1_file.write(f"atom index {i} alpha_1 features:\n")
            for j in range(dim2_):
                alpha_1_file.write(f"neighbor {j}:\n")
                for k in range(dim3_):
                    alpha_1_file.write(str(alpha_1[i][j][k].item()))
                    alpha_1_file.write(' ')
                alpha_1_file.write("\n")

        alpha_1_file.close()

        # print(f"before lin_edge edge_fea_cat.size() = {edge_fea_cat.size()}")

        edge_fea_cat = alpha_1 * self.lin_edge(edge_fea_cat)
        # print(f"after lin_edge edge_fea_cat.size() = {edge_fea_cat.size()}")
...
class CEGAN(nn.Module):
    ...
    def forward(self, data):
        ...
        # print(f"inside model.py: bond_fea.size() = {bond_fea.size()}")
        ...
        # print(f"edge_fea.size() = {edge_fea.size()}")
        ...
        # write the crys_fea to a new file
        num_atom, num_features = crys_fea.shape
        print(f"num_atom = {num_atom} num_features = {num_features}")
        out_file = open('crys_fea.txt', 'a')
        out_file.write(f"num_atom = {num_atom} num_features = {num_features}\n")
        for i in range(num_atom):
            for j in range(num_features):
                out_file.write(str(crys_fea[i][j].item()))
                out_file.write(" ")
            out_file.write("\n")
        out_file.close()

        # print(f"crys_idx = {crys_idx}")
        ...
        # print(out.shape)
        print(f'inside model.py CEGAN_out.size(): {out.size()}')
