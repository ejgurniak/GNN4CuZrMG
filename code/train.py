...
def train(
    ...
    best_val_accuracy, best_val_loss, total_batch_loss, iteration_count = -1e300, 1e300, 0, 0
    ...
    def Checkpoint(epoch, is_best, best_best, path=os.getcwd(), filename="checkpoint.pt"):
    ...
    def log_training_results(engine):
        ...
        if epoch % settings.checkpoint_every == 0:
            Checkpoint(epoch, is_best=False, best_best=False, path=output_dir)
        ...
        if val_accuracy > best_val_accuracy:
            ...
            Checkpoint(
                epoch, is_best=True, best_best=False, path=output_dir
            )  # add checkpoint function here

        if val_accuracy == 1.0 and val_loss < best_val_loss:
            best_val_loss = val_loss
            print_validation_details(model, val_loader, criterion, epoch)
            print(
                "Saving the data for best accuracy {} obtained at epoch {} with val_loss {}".format(
                    best_val_accuracy, epoch, best_val_loss
                )
            )
            Checkpoint(
                epoch, is_best=True, best_best=True, path=output_dir
            )
