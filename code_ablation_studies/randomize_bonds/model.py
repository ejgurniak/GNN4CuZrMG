...
class CEGAN(nn.Module):
    ...
    def forward(self, data):
        ...
        # print(f"bond_fea.shape = {bond_fea.shape}")
        # print(f"angle_fea.shape = {angle_fea.shape}")
        # start block to randomize bond features
        dim1, dim2 = bond_fea.shape
        my_bond_min = 2.510377883911133
        my_bond_max = 4.910924911499023
        for i in range(dim1):
            for j in range(dim2):
                bond_fea[i][j] = random.uniform(my_bond_min, my_bond_max)
        # end block to randomize bond features
        # print(f"bond_fea = {bond_fea}")

        # # start block to randomize angle features
        # dim1, dim2, dim3 = angle_fea.shape
        # my_angle_max = 1.3997753858566284
        # my_angle_min = -1.3549576997756958
        # for i in range(dim1):
        #     for j in range(dim2):
        #         for k in range(dim3):
        #             angle_fea[i][j][k] = random.uniform(my_angle_min, my_angle_max)
        # # end block to randomize angle features
        # print(f"angle_fea = {angle_fea}")
        # my_bond_min = 20.0
        # my_bond_max = -20.0
        # for i in range(dim1):
        #     for j in range(dim2):
        #         if bond_fea[i][j] < my_bond_min:
        #             my_bond_min = bond_fea[i][j]
        #         if bond_fea[i][j] > my_bond_max:
        #             my_bond_max = bond_fea[i][j]
        # # print(f"my_bond_max = {my_bond_max}")
        # # print(f"my_bond_min = {my_bond_min}")
        # dim1, dim2, dim3 = angle_fea.shape
        # my_angle_min = 100.0
        # my_angle_max = -100.0 # these are ok because it is cos(angle), not original angle
        # for i in range(dim1):
        #     for j in range(dim2):
        #         for k in range(dim3):
        #             if angle_fea[i][j][k] < my_angle_min:
        #                 my_angle_min = angle_fea[i][j][k]
        #             if angle_fea[i][j][k] > my_angle_max:
        #                 my_angle_max = angle_fea[i][j][k]
        # # print(f"my_angle_min = {my_angle_min}")
        # # print(f"my_angle_max = {my_angle_max}")
        # out_file = open('bond_min.txt', 'a')
        # out_file.write(str(my_bond_min.item()))
        # out_file.write("\n")
        # out_file.close()
        # out_file = open('bond_max.txt', 'a')
        # out_file.write(str(my_bond_max.item()))
        # out_file.write("\n")
        # out_file.close()
        # out_file = open('angle_min.txt', 'a')
        # out_file.write(str(my_angle_min.item()))
        # out_file.write("\n")
        # out_file.close()
        # out_file = open('angle_max.txt', 'a')
        # out_file.write(str(my_angle_max.item()))
        # out_file.write("\n")
        # out_file.close()

        # out_file = open('bond_fea.txt', 'a')
        # for i in range(dim1):
        #     for j in range(dim2):
        #         out_file.write(str(bond_fea[i][j].item()))
        #         out_file.write(' ')
        #     out_file.write("\n")
        # # out_file.write("\n") # only need one line if we take the average
        # out_file.close()
        # my_bond_avg = torch.mean(bond_fea)
        # print(f"my_bond_avg = {my_bond_avg}")
        # my_angle_avg = torch.mean(angle_fea)
        # print(f"my_angle_avg = {my_angle_avg}")
        # out_file = open('bond_fea_v2.txt', 'a')
        # out_file.write(str(my_bond_avg.item()))
        # out_file.write("\n")
        # out_file.close()
        # out_file = open('angle_fea_v2.txt', 'a')
        # out_file.write(str(my_angle_avg.item()))
        # out_file.write("\n")
        # out_file.close()

        # dim1, dim2, dim3 = angle_fea.shape
        # out_file = open('angle_fea.txt', 'a')
        # for i in range(dim1):
        #     for j in range(dim2):
        #         for k in range(dim3):
        #             out_file.write(str(angle_fea[i][j][k].item()))
        #             out_file.write(' ')
        #     out_file.write("\n")
        # out_file.close()

        # # now read the files you've created
        # list_of_bond_fea = []
        # file = open('bond_fea.txt', 'r')
        # myline = file.readline()
        # mylist = myline.split()
        # list_of_bond_fea.append(mylist)
        # while myline:
        #     myline = file.readline()
        #     mylist = myline.split()
        #     if len(mylist) > 0:
        #         list_of_bond_fea.append(mylist)
        # file.close()
        # # print(f"len(list_of_bond_fea) = {len(list_of_bond_fea)}")
        # # print(f"len(list_of_bond_fea[0]) = {len(list_of_bond_fea[0])}")
        # # print(f"len(list_of_bond_fea[20]) = {len(list_of_bond_fea[20])}")
        # bond_fea_data = torch.zeros(len(list_of_bond_fea), len(list_of_bond_fea[0]))
        # # print(f"bond_fea_data.shape = {bond_fea_data.shape}")
        # dim1, dim2 = bond_fea.shape
        # for i in range(dim1):
        #     for j in range(dim2):
        #         bond_fea_data[i][j]
        ...
        # here we turn off the edge_fea HERE
        # dim1, dim2, dim3 = edge_fea.shape
        # edge_fea = torch.zeros(dim1, dim2, dim3)
        # edge_fea = edge_fea.to(device)
        angle_fea = self.gbf_angle(angle_fea, bond=False)
        # dim1, dim2, dim3, dim4 = angle_fea.shape
        # print(f"dim1 = {dim1}, dim2 = {dim2}, dim3 = {dim3}, dim4 = {dim4}")
        # angle_fea = torch.zeros(dim1, dim2, dim3, dim4)
        angle_fea = angle_fea.to(device)
